...
const ShowProductsByBrands = (props) => {
  // Add to cart method
  const { cart, setCart, addProduct } = useContext(ProductContext);

  const [myData, setMyData] = useState([]);

  const loadBrandPage = (categoryId) => {
    // axios
    ...
    const options = {
      method: "GET",
      url: "http://localhost:4040/products/brands/" + props.id,
    };
  ...
  return (
    ...
    >
      <Row className="mb-3 category-card-main-div justify-content-center justify align-items-center">
        {myData.map((data, key) => {
          return (
            <Col xs={12} sm={6} md={4} lg={3} className="my-4" key={myData._id}>
...
export default ShowProductsByBrands;
